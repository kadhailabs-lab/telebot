...
def rating_keyboard(
    ...
    if has_instagram or has_whatsapp:
        contact_buttons = []
...
async def handle_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    waiting_for_caption[user.id]["caption"] = caption
...
async def handle_instagram(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for prefix in (
        ...
        if instagram.startswith(prefix):
            instagram = instagram[len(prefix):]
            break
    ...
    if share_instagram or share_whatsapp:
        ...
        try:
            ...
            submissions_collection.update_one(
                ...
                {
                    "$set": {
                        "premium_message_id": premium_message.message_id
                    }
                },
            )
...
async def premium_contact(update, context):
    ...
    contact_type = query.data.split(":", 1)[0]
    ...
    if not premium:
        await safe_answer(
            query,
            "🔒 Premium members only.\n\n"
            "Join the Premium channel to view this contact.",
            show_alert=True,
        )
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    "🔒 <b>PREMIUM CONTENT</b>\n\n"
                    "This contact information is available "
                    "to Premium members.\n\n"
                    "💎 Join the Premium channel to view it."
                ),
                parse_mode="HTML",
    ...
    if contact_type == "instagram":
        instagram = submission.get("instagram_id")
    ...
    if contact_type == "whatsapp":
        whatsapp = submission.get("whatsapp_number")
...
async def handle_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        _, submission_id, rating_str = query.data.split(":")
    ...
    average_rating = (
        sum(v["rating"] for v in votes) / vote_count
        if vote_count
        else 0
    )
    ...
    public_caption = (
        ...
        f"⭐ <b>Average:</b> {average_rating:.2f}/5\n"
...
async def leaderboard_popup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for index, member in enumerate(leaderboard):
        ...
        average = member.get("average_rating", 0)
        votes = member.get("vote_count", 0)

        lines.append(
            f"{medals[index]} {name}\n"
            f"   ⭐ {average:.2f}/5 • 🗳 {votes} votes"
        )
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not leaderboard:
        await update.message.reply_text(
            "🏆 No members have reached 10 total votes yet."
        )
        return
    ...
    for index, member in enumerate(leaderboard):
        ...
        lines.append(
            f"{medals[index]} <b>{escape(name)}</b>\n"
            f"⭐ {average:.2f}/5 • 🗳 {votes} votes"
        )
...
async def mystats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    average = (
        sum(v["rating"] for v in votes) / total_votes
        if total_votes
        else 0
    )
